    ...
    <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data">
	    ...
	    <div class="form-group col-md-6">
            ...
            <select name="id_bidang" class="form-control">
                <option value="<?php echo $id_bidang?>">----</option>
                <?php foreach ($gettbbdg as $bidang) { ?>

                    <option value="<?php echo $bidang->id_bidang ?>">
                        <?php echo $bidang->bagian_karyawan ?> 
                    </option>
                <?php } ?>
            </select>

            <label for="int">Cabang</label>
            
            <select name="id_cabang" class="form-control">
                <option value="<?php echo $cabang ?>">----</option>
                <?php foreach ($gettbcabang as $cbg) { ?>

                    <option value="<?php echo $cbg->id_cabang ?>" <?php if($cbg->id_cabang==$bidang){echo "selected"; } ?>>
                        <?php echo $cbg->nama_cabang ?> 
                    </option>
                <?php } ?>
            </select>
